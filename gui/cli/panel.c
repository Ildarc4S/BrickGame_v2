...
void drawText(Panel_t *self) {
    ...
    int start = game_info.next ? PANEL_TETRIS_HELP_TEXT_START : PANEL_SNAKE_HELP_TEXT_START;
    int count = game_info.next ? PANEL_TETRIS_HELP_TEXT_COUNT : PANEL_SNAKE_HELP_TEXT_COUNT;
    for (int i = 0; i < count && (start + i) < self->size; i++) {
        mvprintw(self->y + i, self->x * PANEL_CELL_WIDTH, "%s", self->text[start + i]);
    }
}
...
void _drawPanel(Panel_t *self) {
    ...
    switch (self->mode) {
        case PANEL_MODE_TEXT: drawText(self); break;
        case PANEL_MODE_HIGH_SCORE: mvprintw(self->y, self->x * PANEL_CELL_WIDTH, "%d", game_info.high_score); break;
        case PANEL_MODE_SPEED: mvprintw(self->y, self->x * PANEL_CELL_WIDTH, "%d", game_info.speed); break;
        case PANEL_MODE_LEVEL: mvprintw(self->y, self->x * PANEL_CELL_WIDTH, "%d", game_info.level); break;
        case PANEL_MODE_CUR_SCORE: mvprintw(self->y, self->x * PANEL_CELL_WIDTH, "%d", game_info.score); break;
        case PANEL_MODE_NEXT_FIGURE: drawFigure(self, game_info); break;
    }
}

Panel_t initPanel(int x, int y, const char *title, const char **text, int size, PanelColor_t color, PanelMode_t mode) {
    Panel_t panel = { .x = x, .y = y, .size = size, .mode = mode, .color = color, .draw = _drawPanel };
